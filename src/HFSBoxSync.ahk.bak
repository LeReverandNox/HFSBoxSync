...
FileInstall, Ressources\logo-horizontal.svg, %A_Temp%\HFSBoxSync\logo-horizontal.svg
;~ ---------------------------------------------------------------------------------------------------------
...
FileMove, %A_Temp%\HFSBoxSync\logo-horizontal.svg, %A_ProgramFiles%\Syncthing\gui\assets\img, 1
GuiControl,, MyProgress, +11
...
;~ Si elle n'est pas deja installe, on installe la police dans %systemroot%\Fonts, et on execute FontReg
IfNotExist, %A_WinDir%\Fonts\BebasNeue.ttf
{
	FileMove, %A_Temp%\HFSBoxSync\BebasNeue.ttf, %A_WinDir%\Fonts
...
;~ On supprime la config générée par le -generate
FileDelete, %AppDataLocal%\Syncthing\config.xml
...
else
{
	...
	FileRemoveDir, %AppDataLocal%\Syncthing\index-v0.11.0.db, 1
